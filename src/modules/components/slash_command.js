...
async function slashCommand(options) {
    ...
    if (dir !== false) {
        puts(chalk.bold(chalk.red('\nERROR:'))) && puts(` "${cmd.name}" already exists. Process exited.\n\n`);
    ...
    download(`github:discordjs-cli/${cmd.fw}-boilerplate-slash-command#${cmd.version}`, `src/interactions/slash_commands/${cmd.name}`, {}, async (err) => {
        ...
        try {
            ...
            // Rename file
            await rename(
                `./src/interactions/slash_commands/${cmd.name}/%command_name%.command.${cmd.fw}`,
                `./src/interactions/slash_commands/${cmd.name}/${cmd.name.replace(/ /g, '-')}.command.${cmd.fw}`
            );
